...
class HDRModeContractTests(unittest.TestCase):
    ...
    def test_resolve_mode_accepts_comfyui_hdr_when_hdr_http_configured(self):
        env = {"HDR_HTTP_URL": "http://127.0.0.1:8001/v1/hdr_restore"}
    ...
    def test_resolve_mode_falls_back_without_hdr_http_url(self):
        env = {
            "HDR_HTTP_URL": "",
        ...
        req = app_mod.HdriRequest(image_b64="x", hdr_reconstruction_mode="comfyui_hdr")
        with patch.dict(os.environ, env, clear=False):
            self.assertEqual(app_mod._resolve_hdr_mode(req), "heuristic")

    def test_legacy_ai_fast_request_maps_to_heuristic(self):
        req = app_mod.HdriRequest(image_b64="x", hdr_reconstruction_mode="ai_fast")
        self.assertEqual(req.hdr_reconstruction_mode, "heuristic")
        self.assertEqual(app_mod._resolve_hdr_mode(req), "heuristic")
...
class HDRFailoverTests(unittest.TestCase):
    ...
    @patch.dict("os.environ", {"HDR_FAILOVER_MODE": "heuristic"}, clear=False)
